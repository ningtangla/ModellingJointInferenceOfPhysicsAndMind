...
class TestMCTS(unittest.TestCase):
    ...
    def testRollout(self, max_rollout_step, init_state, gt_sumValue):
        ...
        def rollout_policy(state): return np.random.choice(self.action_space)
        leaf_node = Node(id={1: init_state}, numVisited=1, sumValue=0,
                         actionPrior=self.default_actionPrior, isExpanded=True)
        rollout = RollOut(rollout_policy, max_rollout_step, self.transition,
                          reward_func, isTerminal, nullRolloutHeuristic)
        stored_reward = []
