...
class GenerateModel:
    ...
    def __call__(self, sharedWidths, actionLayerWidths, valueLayerWidths, resBlockSize=0, summaryPath="./tbdata"):
        print(f'Generating NN with shared layers: {sharedWidths}, action layers: {actionLayerWidths}, '
              f'value layers: {valueLayerWidths}, resBlockSize={resBlockSize}')
        graph = tf.Graph()
