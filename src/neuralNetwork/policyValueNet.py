    ...
    def __call__(self, hiddenWidths, summaryPath="./tbdata"):
        ...
        with graph.as_default():
            ...
            with tf.name_scope("hidden"):
                ...
                fcAction = tf.layers.Dense(units=hiddenWidths[-1], activation=tf.nn.relu, kernel_initializer=initWeight,
                                           bias_initializer=initBias)
                aAction_ = fcAction(a_)
                fcLastAction = tf.layers.Dense(units=self.numActionSpace, activation=None,
                ...
                fcValue = tf.layers.Dense(units=hiddenWidths[-1], activation=tf.nn.relu, kernel_initializer=initWeight,
                                          bias_initializer=initBias)
                aValue_ = fcValue(a_)
                fcLastValue = tf.layers.Dense(units=1, activation=None, kernel_initializer=initWeight,
                                              bias_initializer=initBias)
            ...
            with tf.name_scope("outputs"):
                ...
                relativeValueError_ = tf.cast((tf.abs((valuePrediction_ - valueLabel_) / valueLabel_)),
                                              relativeErrorBound_.dtype)
...
class Train:
    def __init__(self, maxStepNum, batchSize, terminalController, coefficientController, trainReporter):
    ...
    def __call__(self, model, trainingData):
        ...
        for stepNum in range(self.maxStepNum):
            ...
            evalDict, _, summary = model.run(fetches, feed_dict={state_: stateBatch, actionLabel_: actionLabelBatch,
                                                                 valueLabel_: valueLabelBatch,
                                                                 actionLossCoef_: actionLossCoef,
...
def evaluate(model, testData, summaryOn=False, stepNum=None):
    ...
    fetches = [
        {"actionLoss": actionLoss_, "actionAcc": actionAccuracy_, "valueLoss": valueLoss_, "valueAcc": valueAccuracy_},
        evalSummaryOp]

    stateBatch, actionLabelBatch, valueLabelBatch = testData
    evalDict, summary = model.run(fetches, feed_dict={state_: stateBatch, actionLabel_: actionLabelBatch,
                                                      valueLabel_: valueLabelBatch})
    if summaryOn:
