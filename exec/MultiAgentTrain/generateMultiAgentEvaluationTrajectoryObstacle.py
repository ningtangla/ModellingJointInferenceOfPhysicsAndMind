...
def main():
    ...
    killzoneRadius = 2
    trajectoryFixedParameters = {'maxRunningSteps': trainMaxRunningSteps, 'numSimulations': trainNumSimulations,
    ...
    if not os.path.isfile(trajectorySavePath):
        ...
        multiAgentNNmodel = [generateModel(sharedWidths, actionLayerWidths, valueLayerWidths) for agentId in
                             range(numAgents)]
        ...
        getResetFromTrial = lambda trial: ResetUniform(physicsSimulation, evalAllQPosInit[trial],
                                                       evalAllQVelInit[trial],
                                                       numAgents, evalQPosInitNoise, evalQVelInitNoise)
        evalMaxRunningSteps = 30
        ...
        restoredMultiAgentNNModel = [restoreNNModel(agentId, multiAgentIterationIndex[agentId]) for agentId in
                                     range(numAgents)]
