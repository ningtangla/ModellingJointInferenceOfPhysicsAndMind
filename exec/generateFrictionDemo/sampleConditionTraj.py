...
class ApproximatePolicy:
    def __init__ (self, policyValueNet, actionSpace, agentStateIdsForNN, coordinateIndexForNN):
        self.policyValueNet = policyValueNet
        self.actionSpace = actionSpace
        self.agentStateIdsForNN = agentStateIdsForNN
        self.coordinateIndexForNN = coordinateIndexForNN

    def __call__(self, state):
        stateNN = np.array([state[agentId][self.coordinateIndexForNN] for agentId in self.agentStateIdsForNN])
...
class ApproximateValue:
    def __init__(self, policyValueNet, agentStateIdsForNN, coordinateIndexForNN):
        self.policyValueNet = policyValueNet
        self.agentStateIdsForNN = agentStateIdsForNN
        self.coordinateIndexForNN = coordinateIndexForNN
...
def main():
    ...
    dirName = os.path.dirname(__file__)
    trajectoriesSaveDirectory = os.path.join(dirName, '..', '..', 'data','generateFrictionDemo','trajectories')
    ...
    friction = float(parametersForTrajectoryPath['friction'])
    ...
    if not os.path.isfile(trajectorySavePath):
        ...
        physicsSimulation.model.geom_friction[:, 0] = friction
        ...
        numAgent = 3
        numRopePart = 9
        qPosInit = (0, ) * 3 * (numAgent + numRopePart)
        qVelInit = (0, ) * 3 * (numAgent + numRopePart)
        qPosInitNoise = [6, 6, 0]
        qVelInitNoise = [3, 3, 0]
        tiedAgentId = [1, 2]
...
# neural network init
        actionSpace = [(10, 0, 0), (7, 7, 0), (0, 10, 0), (-7, 7, 0), (-10, 0, 0), (-7, -7, 0), (0, -10, 0), (7, -7, 0)]
...
# multAgent ApproximatePolicyAndActionPrior
        ...
        coordinateIndexForNN = [0, 1, 3, 4, 6, 7]
        ...
        getApproximatePolicy = lambda NNmodel, actionSpace, agentStateIdsForNN: ApproximatePolicy(NNmodel, actionSpace, agentStateIdsForNN, coordinateIndexForNN)
