...
def main():
    ...
    agentMaxSize=0.2
    wallList=[[0,5,1.125,1.5],[0,-5,1.125,1.5]]
    ...
    # sample trajectory
    sampleTrajectory = SampleTrajectory(maxRunningSteps, transit, isTerminal, reset, [chooseGreedyAction,chooseGreedyAction])
    ...
    trajectoriesSaveDirectory = os.path.join(dirName, '..', '..', 'data','multiAgentTrain', 'multiMCTSAgentObstacleOriginEnvParallel', 'trajectories')
    if not os.path.exists(trajectoriesSaveDirectory):
        os.makedirs(trajectoriesSaveDirectory)

    NNModelSaveDirectory = os.path.join(dirName, '..', '..', 'data','multiAgentTrain', 'multiMCTSAgentObstacleOriginEnvParallel', 'NNModel')
    ...
    numTrajectoriesPerIteration = 16
    numIterations = 100000
    saveCheckPointInterval = 50

    sampleTrajectoryFileNameIntrain = 'sampleMultiMCTSAgentTrajectoryObstacleIntrain.py'
    numCpuCores = os.cpu_count()
    numCpuToUse = int(0.8 * numCpuCores)
    numCmdList = min(numTrajectoriesPerIteration, numCpuToUse)
    generateTrajectoriesParallelIntrain = GenerateTrajectoriesParallel(sampleTrajectoryFileNameIntrain, numTrajectoriesPerIteration,numCmdList)

    fuzzySearchParameterNames = ['sampleIndex']
    loadTrajectoriesForParallelIntrain = LoadTrajectories(generateTrajectorySavePath, loadFromPickle,fuzzySearchParameterNames)
    ...
    for iterationIndex in range(restoredIteration + 1, numIterations):
        ...
        trajectoryPathParameters={'iterationIndex': iterationIndex}
        cmdList = generateTrajectoriesParallelIntrain(trajectoryPathParameters)
        trajectorySavePath = generateTrajectorySavePath(trajectoryPathParameters)
