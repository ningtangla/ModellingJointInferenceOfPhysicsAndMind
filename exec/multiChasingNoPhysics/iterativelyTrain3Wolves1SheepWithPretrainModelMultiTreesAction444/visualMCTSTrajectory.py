...
def main():
    ...
    trajectoryDirectory = os.path.join(dirName, '..', '..', '..', 'data', 'multiAgentTrain', 'multiMCTSAgentResNetNoPhysicsCenterControlWithPreTrain', 'trajectories')
    ...
    stateIndex=0
    darwTrajectory = DrawTrajectoryWithRender(stateIndex,render)


    for dataIndex in range(len(allTrajectories)):
        trajectory = allTrajectories[dataIndex]
        del trajectory[-1]
        darwTrajectory(trajectory)
