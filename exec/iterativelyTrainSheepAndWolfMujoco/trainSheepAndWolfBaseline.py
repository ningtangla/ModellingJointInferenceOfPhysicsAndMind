...
def main():
    ...
    sheepActionToOneHot = lambda action: np.asarray([1 if (np.array(action) == np.array(sheepActionSpace[index])).all() else 0
                                                     for index in range(len(sheepActionSpace))])
    wolfActionToOneHot = lambda action: np.asarray([1 if (np.array(action) == np.array(wolfActionSpace[index])).all() else 0
                                                    for index in range(len(wolfActionSpace))])
    actionIndex = 1

    preProcessSheepTrajectories = PreProcessTrajectories(sheepId, actionIndex, sheepActionToOneHot)
    sheepStateActionPairsProcessed = preProcessSheepTrajectories(dataSetTrajectories)
    ...
    preProcessWolfTrajectories = PreProcessTrajectories(wolfId, actionIndex, sheepActionToOneHot)
    wolfStateActionPairsProcessed = preProcessWolfTrajectories(dataSetTrajectories)
...
# train sheep
    allTrainedSheepModels = {trainSteps: train(generatePolicyNet(hiddenWidths), sheepTrainData) for trainSteps, train in
    ...
    allSheepModelSavePaths = {trainedModel: getSheepModelSavePath({'trainSteps': trainSteps}) for trainSteps, trainedModel in
                              allTrainedSheepModels.items()}
...
# train wolf
    ...
    getWolfModelSavePath = GetSavePath(wolfNNModelSaveDirectory, NNModelSaveExtension, NNFixedParameters)
    allWolfModelSavePaths = {trainedModel: getWolfModelSavePath({'trainSteps': trainSteps}) for trainSteps, trainedModel in
                             allTrainedSheepModels.items()}
