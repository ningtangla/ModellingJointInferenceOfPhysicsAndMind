...
def main():
    ...
    if not os.path.isfile(trajectorySavePath):
        ...
        masterModelPath = os.path.join('..', '..', 'data', 'evaluateSupervisedLearning', 'leashedWolfNNModels','agentId=1_depth=4_learningRate=0.0001_maxRunningSteps=25_miniBatchSize=256_numSimulations=200_trainSteps=20000')
        restoredMasterModel = restoreVariables(initMasterNNModel, masterModelPath)
        masterPolicy = ApproximatePolicy(restoredMasterModel, masterActionSpace)
       
        sheepModelPath = os.path.join('..', '..', 'data', 'evaluateSupervisedLearning', 'leashedWolfNNModels','agentId=1_depth=4_learningRate=0.0001_maxRunningSteps=25_miniBatchSize=256_numSimulations=200_trainSteps=20000')
        restoredWolfModel = restoreVariables(initWolfNNModel, wolfModelPath)
        wolfPolicy = ApproximatePolicy(restoredWolfModel, wolfActionSpace)
        ...
        #mcts = StochasticMCTS(numSimulationsPerTree, selectChild, expand, rollout, backup, establishPlainActionDistFromMultipleTrees)
        numSimulations = 200
        #mcts = StochasticMCTS(numSimulationsPerTree, selectChild, expand, rollout, backup, establishPlainActionDistFromMultipleTrees)
