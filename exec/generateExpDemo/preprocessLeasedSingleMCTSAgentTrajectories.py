...
def main():
    ...
    masterDelayStep = 10
    minLength = 240 + masterDelayStep
    timestepCheckInterval = 20
    checkTimesteps = np.arange(0, maxRunningSteps - minLength, timestepCheckInterval)
    ...
    for pathParameters in conditionParametersAll:
        ...
        for startCheckTimestep in checkTimesteps:
            ...
            if endCheckTimeStep <= maxRunningSteps:

                pathParameters.update({'timeStep': (startCheckTimestep, endCheckTimeStep)})
                trajectories = [trajectory[startCheckTimestep:endCheckTimeStep] for trajectory in originalTrajectories]
                stateIndex = 0
                qPosIndex = [0, 1]
