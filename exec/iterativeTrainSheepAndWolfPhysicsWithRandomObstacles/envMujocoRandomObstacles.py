...
class ResetUniformInEnvWithObstacles:
    ...
    def __call__(self,obstacleProperty):
        ...
        while self.checkAngentStackInWall(qPos,obstacleProperty):
            qPos = self.qPosInit + np.random.uniform(low=-self.qPosInitNoise, high=self.qPosInitNoise, size=numQPos)
        ...
        startState=np.asarray(agentState+obstacleProperty)
        return startState
...
class TransitionFunction:
    ...
    def __call__(self, state, actions):
        ...
        obstacleProperty=[obstacle for obstacle in state[self.numAgents:]]
        ...
        for simulationFrame in range(self.numSimulationFrames):
            ...
            agentNewState=[getAgentNewState(agentIndex) for agentIndex in range(numAgent)]
