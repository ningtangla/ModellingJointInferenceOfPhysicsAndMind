...
from src.constrainedChasingEscapingEnv.measurementFunctions import DistanceBetweenActualAndOptimalNextPosition,ComputeOptimalNextPos
from src.constrainedChasingEscapingEnv.wrapperFunctions import GetAgentPosFromState, GetAgentPosFromTrajectory,GetStateFromTrajectory

import src.constrainedChasingEscapingEnv.envNoPhysics as env
from src.algorithms.mcts import MCTS, CalculateScore, selectGreedyAction, SelectChild, Expand, RollOut, backup, \
    InitializeChildren
from src.constrainedChasingEscapingEnv.policies import HeatSeekingContinuesDeterministicPolicy,stationaryAgentPolicy,HeatSeekingDiscreteDeterministicPolicy
...
if __name__ == "__main__":
    ...
    manipulatedVariables['numSimulations'] = [200, 500, 800]  # [10, 20, 30]
    ...
    def sheepTransit(state, action): return transitionFunction(
        state, [action, stationaryAgentPolicy(state)])
    ...
    print("Init position restored from {}".format(savedPositionFileName))
    ...
    trajectoryDirectory = "../../data/evaluateNNPriorMCTSNoPhysicsSheepChaseWolf/trajectories"
    ...
    getOptimalAction = HeatSeekingDiscreteDeterministicPolicy(actionSpace, getPredatorPos, getPreyPos, computeAngleBetweenVectors)
    computeOptimalNextPos = ComputeOptimalNextPos(getInitStateFromTrajectory, getOptimalAction, sheepTransit, getPreyPos)
    measurementTimeStep = 1
    getNextStateFromTrajectory = GetStateFromTrajectory(measurementTimeStep, stateIndex)
    getPosAtNextStepFromTrajectory = GetAgentPosFromTrajectory(getPreyPos, getNextStateFromTrajectory)
