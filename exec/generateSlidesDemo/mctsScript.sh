...
do
    ...
    do
        ...
        do 
            ...
            ffmpeg -r 120 -f image2 -s 1920x1080 -i ~/ModellingJointInferenceOfPhysicsAndMind/data/generateMCTSDemo/image/beta=${beta}_index=${index}_killzoneRadius=0.5_masterPowerRatio=${masterPowerRatio}_maxRunningSteps=9_numAgents=3_numSimulations=80_pureMCTSAgentId=${pureMCTSAgentId}.pickle/image/%0d.png -vcodec libx264 -crf 25  -pix_fmt yuv420p ~/ModellingJointInferenceOfPhysicsAndMind/data/generateMCTSDemo/demo/beta=${beta}_index=${index}_killzoneRadius=0.5_masterPowerRatio=${masterPowerRatio}_maxRunningSteps=9_numAgents=3_numSimulations=80_pureMCTSAgentId=${pureMCTSAgentId}_Demo${index}.mp4
