...
#         distractorPreTrainModel = restoreVariables(initdistractorNNModel, distractorPreTrainModelPath)
        ...
        transitInWolfMCTSSimulation = \
                lambda state, wolfSelfAction: transit(state, [chooseGreedyAction(sheepPolicy(state[:4])), wolfSelfAction, chooseGreedyAction(masterPolicy(state[:4])),
...
# MCTS wolf
        ...
        policy = lambda state: [sheepPolicy(state[:4]), mcts(state),  masterPolicy(state[:4]), distractorPolicy(state[:4])]
